...
export const TBRenderer = (Base) => class extends Base {
    renderInternal(color, opacityFactor) {
        ...
        drawer.setProperty('globalAlpha', 0.9 * opacityFactor);
        drawer.setProperty('lineWidth', 3.25 * z);
        drawer.beginPath()
            // (front wheel)
            .arc(backWheel.x, backWheel.y, 11 * z, 0, 2 * Math.PI, true)
            // (back wheel)
            .moveTo(frontWheel.x + 11 * z, frontWheel.y).arc(frontWheel.x, frontWheel.y, 11 * z, 0, 2 * Math.PI, true).stroke();
        ...
        drawer.setProperty('lineWidth', 1.75 * z);
        drawer.setProperty('globalAlpha', 0.35 * opacityFactor)
        drawer.beginPath()
            // (front wheel)
            .arc(backWheel.x, backWheel.y, 8.5 * z, 0, 2 * Math.PI, true)
            // (back wheel)
            .moveTo(frontWheel.x + 8.5 * z, frontWheel.y).arc(frontWheel.x, frontWheel.y, 8.5 * z, 0, 2 * Math.PI, true).stroke();
        */
        // Frame
        drawer.setProperty('globalAlpha', opacityFactor);
        drawer.setProperty('lineWidth', 3.25 * z);
        ...
        let CY = new Vector(6.5 * z * Math.cos(this.distance), 6.5 * z * Math.sin(this.distance)),
            pedal = pedalHinge.add(CY),
            shadowPedal = pedalHinge.sub(CY),
            saddle = backWheel.add(length.scale(0.18)).add(AC.scale(0.42)),
            Cg = backWheel.add(length.scale(0.3)).add(AC.scale(0.42)),
        ...
        let knee = hip.add(Ar.scale(0.5)).add(BA.scale(250 / Ar.lengthSquared()));
        ...
        // Shadow Leg
        drawer.beginPath()
            .setProperty('lineWidth', 4.5 * z)
            .setProperty('globalAlpha', 0.65 * opacityFactor)
            .moveTo(shadowPedal.x-z, shadowPedal.y).lineTo(shadowKnee.x, shadowKnee.y).lineTo(hip.x, hip.y).stroke();
        // Leg
        drawer.beginPath()
            .setProperty('globalAlpha', opacityFactor)
            .moveTo(pedal.x, pedal.y-z).lineTo(knee.x, knee.y).lineTo(hip.x, hip.y).stroke();
        // Body
        head = crossFrameSaddle.add(length.scale(0.05)).add(AC.scale(0.9));
        drawer.beginPath()
            .setProperty('lineWidth', 7 * z)
            .moveTo(hip.x, hip.y).lineTo(head.x, head.y+2*z).stroke();
        // Head
        let headCenter = crossFrameSaddle.add(length.scale(0.15)).add(AC.scale(1.065));
        drawer.setProperty('lineWidth', 2 * z);
        drawer.beginPath().moveTo(headCenter.x + 5.5 * z, headCenter.y).arc(headCenter.x, headCenter.y, 5.5 * z, 0, 2 * Math.PI, true).stroke();
        // Cap
        drawer.beginPath();
        switch (this.cap) {
            /*
            case 'cap':
                let capFront = crossFrameSaddle.add(length.scale(0.4)).add(AC.scale(1.12)),
                    capBack = crossFrameSaddle.add(length.scale(0.05)).add(AC.scale(1.07));
                drawer.moveTo(capBack.x, capBack.y).lineTo(capFront.x, capFront.y).stroke();
                break;
            case 'hat':
                let hatFrontBottom = crossFrameSaddle.add(length.scale(0.35)).add(AC.scale(1.17)),
                    hatBackBottom = crossFrameSaddle.sub(length.scale(0.05)).add(AC.scale(1.12)),
                    hatFront = crossFrameSaddle.add(length.scale(0.25)).add(AC.scale(1.15)),
                    hatBack = crossFrameSaddle.add(length.scale(0.05)).add(AC.scale(1.138)),
                    hatFrontTop = hatFrontBottom.sub(length.scale(0.1)).selfAdd(AC.scale(0.24)),
                    hatBackTop = hatBackBottom.add(length.scale(0.02)).selfAdd(AC.scale(0.24));
