...
import { TBGhost } from "../bike/ghost/TBGhost.js";
import { BIKE_BMX, BIKE_HAR, BIKE_MTB, BIKE_TB } from "../constant/BikeConstants.js";
...
export class RaceTrack extends Track {
    constructor(ID, canvas, game) {
        ...
        if (hashSplit[3] === BIKE_BMX || hashSplit[3] === BIKE_MTB || hashSplit[3] === BIKE_HAR || hashSplit[3] === BIKE_TB) {
    ...
    restart() {
        ...
        for (let i = 0; i < this.ghostKeys.length; i++) {
            this.ghostInstances[i] = ghost =
                new(this.ghostKeys[i][6] === BIKE_BMX ? BMXGhost : (this.ghostKeys[i][6] === BIKE_MTB ? MTBGhost : TBGhost))(this, this.ghostKeys[i], cp && cp.ghostLists[i]);
